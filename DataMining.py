...
## Carregando os dados
dataset = pd.read_csv('/content/BASE_MARKETING.TXT',sep='\t') # Separador TAB
...
dataset['pre_vl_totl_rend'] = [(x-0)/(10000-0) for x in dataset['pre_vl_totl_rend']] 
# ATRIBUTO CD_SEXO
dataset['pre_cd_sexo']   = [1 if x == 'M' else 0 for x in dataset['CD_SEXO']]
# ATRIBUTO SG_UF
dataset['pre_uf_sp']   = [1 if x == 'SP' else 0 for x in dataset['SG_UF']]
dataset['pre_uf_rj']   = [1 if x == 'RJ' else 0 for x in dataset['SG_UF']]
dataset['pre_uf_go']   = [1 if x == 'GO' else 0 for x in dataset['SG_UF']]
dataset['pre_uf_mg']   = [1 if x == 'MG' else 0 for x in dataset['SG_UF']]
dataset['pre_uf_ba']   = [1 if x == 'BA' else 0 for x in dataset['SG_UF']]
dataset['pre_uf_pr']   = [1 if x == 'PR' else 0 for x in dataset['SG_UF']]
dataset['pre_uf_rs']   = [1 if x == 'RS' else 0 for x in dataset['SG_UF']]
dataset['pre_uf_sc']   = [1 if x == 'SC' else 0 for x in dataset['SG_UF']]
dataset['pre_uf_pe']   = [1 if x == 'PE' else 0 for x in dataset['SG_UF']]
dataset['pre_uf_0']   = [1 if x == '' else 0 for x in dataset['SG_UF']]
# ---------------------------------------------------------------------------
...
selected = RFE(model,step=1,n_features_to_select=18).fit(X_train, y_train)
